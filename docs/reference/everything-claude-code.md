...
# Everything Claude Code Reference / Everything Claude Code 参考

## 参考仓库 / Reference Repository
...
## 和 Superpowers / gstack 的区别 / Differences From Superpowers And gstack
...
## 最值得关注的点 / Most Worthwhile Points

### 1. Skills 组织方式 / How Skills Are Organized
...
### 3. Rules + Commands + Agents 的分层组织 / Layering Of Rules, Commands, And Agents
...
### 4. MCP 配置作为正式子系统 / MCP Config As A First-Class Subsystem
...
## 它对我们最可用的借鉴点 / Most Useful Takeaways For Us

如果只挑最值得借的 4 个点，我会选：

### 1. `skills/` 层 / The `skills/` Layer

我们未来很适合引入自己的技能目录和技能资产组织方式。

### 2. hooks / middleware 插槽 / Hooks And Middleware Slots

把系统级行为统一挂到可扩展插槽，而不是分散在多个入口。

### 3. command / agent / context / schema 分层 / Layering Of Command, Agent, Context, And Schema

让“工程资产”有稳定位置，而不是把所有能力写死在 loop 或 CLI 里。

### 4. `mcp-configs/` 作为正式目录 / `mcp-configs/` As A First-Class Directory

这可以成为我们未来外部信息接入和工具接入层的自然落点。

## 当前我们明显缺少的对应层 / What We Are Clearly Missing Right Now

如果用 ECC 来照我们当前项目，最明显的缺口是：

### 1. 缺统一的 skills 层 / Missing A Unified Skills Layer

- 没有 `skills/`
- 没有技能发现与组织方式
- 没有技能和 workflow 的正式耦合层

### 2. 缺 hooks / middleware 框架 / Missing A Hooks And Middleware Framework

- 系统能力还没有统一成标准插槽

### 3. 缺 commands / agent assets 层 / Missing A Commands And Agent Assets Layer

- 目前有 CLI，但还没有成体系的 command assets / agent assets

### 4. 缺 MCP 作为正式子系统 / Missing MCP As A First-Class Subsystem
...
## 现在不要急着全量照搬的部分 / What Not To Copy Too Early
