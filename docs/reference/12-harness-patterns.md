...
# 12 Harness Patterns Reference / 12 个 Harness 模式参考
...
## 文章在讲什么 / What The Article Covers
...
## 这篇文章最值得吸收的地方 / What Is Most Worth Absorbing

如果只提炼最关键的 4 个观点，我会总结成这些：

### 1. 上下文必须是作用域化的 / Context Must Be Scoped

不是把整个仓库都塞进 prompt，而是只把和当前任务最相关的部分组织进去。

### 2. 成功标准不能只靠模型主观判断 / Success Criteria Cannot Rely On Model Judgment Alone

需要有显式的验证门、完成定义和失败信号。

### 3. 工具与权限必须有外部控制层 / Tools And Permissions Need An External Control Plane

不能假设模型自己会一直“有分寸”。

### 4. 失败恢复是正常路径，不是异常分支 / Failure Recovery Is A Normal Path, Not An Exception Path

repair loop 不是附加项，而是 Harness 的一部分。

## 和 `Claude_Code_MVP` 的对照 / Mapping To `Claude_Code_MVP`
...
## 一、已经有了第一版实现的 / I. Already Implemented In A First Version

### 1. Persistent Instruction Files / 持久指令文件
...
### 2. Scoped Context Assembly / 作用域上下文组装
...
### 3. Explore -> Plan -> Execute / 探索到计划再到执行
...
### 6. Least-Privilege / Permission Gate / 最小权限与权限门
...
## 二、已经有雏形，但还比较轻的 / II. Present As Light First Shapes

### 9. Layered Memory / 分层记忆
...
### 10. Progressive Context Compression / 渐进式上下文压缩
...
### 11. Explicit Stage Machine / 显式阶段机
...
## 三、还明显缺失的 / III. Still Clearly Missing

### 12. Parallel / Multi-Agent Workflow / 并行与多 Agent 工作流

说明：

当前项目明确还是单代理优先，没有真正的 sub-agent orchestration、上下文隔离和并行执行控制。

### 13. Dreaming / Background Memory Consolidation / 背景记忆整合

说明：

文章提到的后台记忆整合、摘要合并、历史压缩，这类能力当前还没有。

### 14. Headless Batch / Productized Unattended Mode / 无头批处理与产品化无人值守模式

说明：

虽然项目已经有 CLI 和 verify 脚本，但还没有真正产品化的无头批处理运行面。

## 对本项目的直接结论 / Direct Conclusion For This Project
...
## 还最值得继续补的地方 / What Is Still Most Worth Building Next

如果继续沿着这篇文章的思路推进，后面最值得补的 4 个方向是：

### 1. 把 loop 继续抽成 phase-aware state machine / Continue Extracting The Loop Into A Phase-Aware State Machine
...
### 2. 把 memory 从 replay 雏形升级成分层记忆系统 / Upgrade Memory From Replay Prototype To A Layered Memory System

### 3. 把上下文选择继续推进到压缩与预算管理 / Push Context Selection Toward Compression And Budgeting

### 4. 引入更明确的多 agent / worktree isolation 模式 / Introduce A Clearer Multi-Agent And Worktree Isolation Model
